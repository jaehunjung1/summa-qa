...
class QA_Bert():
    def __init__(self):
        ...
        self.model = BertForQuestionAnswering.from_pretrained('bert-large-uncased-whole-word-masking-finetuned-squad').to("cuda:0")
    ...
    def predict(self, question, text):
        ...
        start_scores, end_scores = self.model(torch.tensor([input_ids]).to("cuda"))

        start_scores = torch.functional.F.softmax(start_scores, -1) * torch.Tensor(token_type_ids).to("cuda")
        end_scores = torch.functional.F.softmax(end_scores, -1) * torch.Tensor(token_type_ids).to("cuda")
